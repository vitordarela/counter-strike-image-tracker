...
const CONFIG = {
	...
	ITEMS_API_BASE_URL: "https://vitordarela.github.io/CSGO-API/api/en",
...
function ensureStaticDir() {
	if (!fs.existsSync(CONFIG.STATIC_DIR)) {
		fs.mkdirSync(CONFIG.STATIC_DIR, { recursive: true });
	}
}

// Image tracker class to encapsulate functionality
class ImageTracker {
	constructor() {
...
async function setPhaseItemsToNull() {
	...
	try {
		...
		for (const item of allItems) {
			if (tracker.existingImageUrls[item.image_inventory] !== null) {
				tracker.existingImageUrls[item.image_inventory] = null;
				updatedCount++;
			}
		}

		if (updatedCount > 0) {
			tracker.saveImageUrls();
...
async function main() {
	...
	try {
		await tracker.login(process.argv[2], process.argv[3]);
		await tracker.run();
	} catch (error) {
